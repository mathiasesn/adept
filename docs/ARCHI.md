...
**A third rule flavour: `ParseErrorRule`.** A skill with malformed frontmatter has no `Skill` to run `SkillRule`s against, so `SL001`/`SL002`/`SL003` are also registered as `ParseErrorRule`s (`fn check(&self, path: &Path, err: &AdeptError) -> Vec<Diagnostic>`) in `Registry::parse_error_rules`. `SL001`/`SL002` are dual-registered — once as a `SkillRule` (the field is present but empty) and once as a `ParseErrorRule` (the field is missing entirely, so parsing itself failed); `SL003` (`malformed-frontmatter`) exists only as a `ParseErrorRule`, since malformed frontmatter always fails to parse. `Linter::lint_set` runs `parse_error_rules` over `SkillSet::errors` through the same `is_enabled` + `apply_overrides` dispatch as `skill_rules`/`set_rules`, so `--ignore`/`disabled`/`severity_overrides` work by code or kebab-case name exactly as they do for every other rule. `Registry::meta_iter` dedupes by code so the dual registration of `SL001`/`SL002` doesn't produce duplicate `RuleMeta` entries in `all_meta`/`by_code`/`by_name`.
...
- **`check_skill` uses `LintConfig::default()` and does not discover `adept.toml`.** This is intended, not an oversight: the CLI stays the only config-aware entry point, so an MCP client's results can't silently shift depending on what config file happens to sit near a path it passes in. Pinned by a test in `crates/adept_cli/src/commands/mcp.rs` that raises `description_min_tokens` in a sibling `adept.toml` and asserts `check_skill` still lints against the default threshold.
