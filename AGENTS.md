...
- **adept spawns no subprocess, ever.** Not `check`/`fmt`/`eval`/`fix`, not `create` (the `command` eval assertion is defined and validated, never executed). Eval-dataset grading judges a `command` assertion only from a harness-supplied exit code. This is about the Rust tool; the Python distribution shim (`python/adept/__main__.py`) is the one deliberate, spec-mandated exception: its POSIX path uses `os.execvp`, which replaces the process rather than spawning one, and only its Windows branch uses `subprocess.run` (no POSIX equivalent exists for exec-and-replace on Windows).
