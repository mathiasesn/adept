...
use crate::prompts::{render, OVERLAP_ADJUDICATION_SYSTEM, OVERLAP_ADJUDICATION_USER_TEMPLATE};
...
///
/// # Errors
/// Returns [`ScoreError`] if the LLM client errors or a response cannot be
...
pub async fn detect_overlaps(
    ...
    model: &str,
    threshold: f64,
) -> Result<Vec<OverlapAdjudication>, ScoreError> {
    ...
    for candidate in candidates {
        ...
        let parsed: RawAdjudication = serde_json::from_str(&response.content)
            .map_err(|e| ScoreError::MalformedLlmJson(format!("overlap adjudication: {e}")))?;
