...
///
/// # Errors
/// Returns [`ScoreError`] if the LLM client errors or its response cannot
...
pub async fn analyze_token_bloat(
    ...
    model: &str,
) -> Result<TokenBloatReport, ScoreError> {
    ...
    let parsed: RawSuggestions = serde_json::from_str(&response.content)
        .map_err(|e| ScoreError::MalformedLlmJson(format!("token bloat suggestions: {e}")))?;
...
mod tests {
    ...
    async fn malformed_json_is_a_score_error_not_a_panic() {
        ...
        let err = analyze_token_bloat(&mock, &skill, &counter, "test-model")
            .await
            .unwrap_err();
        assert!(matches!(err, ScoreError::MalformedLlmJson(_)));
