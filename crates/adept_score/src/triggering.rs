...
///
/// # Errors
/// Returns [`ScoreError`] if the LLM client errors, or if a response cannot
/// be parsed as the expected JSON shape.
pub async fn score_triggering(
    client: &dyn LlmClient,
    skill: &Skill,
    options: &TriggeringOptions,
) -> Result<TriggeringReport, ScoreError> {
...
async fn generate_prompts(
    ...
    skill: &Skill,
    options: &TriggeringOptions,
) -> Result<Vec<CandidatePrompt>, ScoreError> {
    ...
    let parsed: RawPromptList = serde_json::from_str(&response.content)
        .map_err(|e| ScoreError::MalformedLlmJson(format!("prompt generation: {e}")))?;
...
async fn judge_prompt(
    ...
    options: &TriggeringOptions,
) -> Result<PromptJudgement, ScoreError> {
    ...
    for _ in 0..samples {
        ...
        let parsed: RawJudgement = serde_json::from_str(&response.content)
            .map_err(|e| ScoreError::MalformedLlmJson(format!("trigger judge: {e}")))?;
